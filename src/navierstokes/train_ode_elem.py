...
if __name__ == "__main__":
    ...
    if not args.test_only:
        ...
        for iteration in range(args.epochs):
            ...
            for i in range(grid_dim):
                for j in range(grid_dim):
                    ij = i * grid_dim + j
                    batch_obs_pred_ij = odeint(model[ij], batch_obs0[:, :, i, j], batch_t)
            ...
            if iteration % 10 == 0 and iteration > 0:
                ...
                with torch.no_grad():
                    ...
                    for i in range(grid_dim):
                        for j in range(grid_dim):
                            ij = i * grid_dim + j
                            val_obs_pred_ij = odeint(model[ij], val_obs0[:, :, i, j], t)
                            val_loss_ij = torch.mean(torch.pow(
                                val_obs_pred_ij -  val_obs[:, :, :, i, j], 2))
    ...
    with torch.no_grad():
        ...
        for i in range(grid_dim):
            for j in range(grid_dim):
                ij = i * grid_dim + j
                pred_obs_ij = odeint(model[ij], obs0[:, :, i, j], t)
